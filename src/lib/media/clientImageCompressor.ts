/**
 * 📷 Client-Side Heirloom Photo Compression & Memory Shield Engine
 *
 ...
 *
 * Features:
 * - Proportional bounding downsampling (default max 1600px edge, no upscaling)
 * - Controlled JPEG compression (default 0.82 quality) for optimal clarity vs payload
 * - Mobile Safari Memory Protection: Image.decode() + try/finally URL.revokeObjectURL()
 * - WebKit GPU texture release: Synchronous 1×1 canvas buffer collapse
 * - Human-readable file size formatting in British English
 */
...
/**
 * Calculates scaled dimensions ensuring images never exceed maxBounding (default 1600px)
 * and are never upscaled beyond their original natural resolution.
 * Preserves aspect ratio across landscape, portrait, and square photographs.
 */
export function calculateScaledDimensions(
  ...
  // Preserve natural dimensions if photograph is already within bounding limits (no upscaling)
 ...
 *
 * Honours Rule 20 UK English orthography and Rule 26 elder ergonomics.
 */
