...
- **Hard Code-Edit Lockout**: If a task requires a higher model tier, the agent is **STRICTLY FORBIDDEN** from invoking code-modification tools (`replace_file_content`, `multi_replace_file_content`, `write_to_file`, `run_command` git commit). It MUST immediately halt and output ONLY the ready-to-paste prompt.
